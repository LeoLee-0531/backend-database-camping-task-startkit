...
UPDATE "USER" SET "role" = 'COACH' WHERE "name" IN ('李燕容', '肌肉棒子', 'Q太郎');
...
VALUES
(
	(SELECT c.id FROM "COACH" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = '李燕容'),
	(SELECT id FROM "SKILL" WHERE name = '重訓')
),
(
	(SELECT c.id FROM "COACH" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = '肌肉棒子'),
	(SELECT id FROM "SKILL" WHERE name = '重訓')
),
(
	(SELECT c.id FROM "COACH" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = '肌肉棒子'),
...
(
	(SELECT c.id FROM "COACH" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = 'Q太郎'),
	(SELECT id FROM "SKILL" WHERE name = '重訓')
),
(
	(SELECT c.id FROM "COACH" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = 'Q太郎'),
...
WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明') 
AND course_id = (SELECT c.id FROM "COURSE" c INNER JOIN "USER" u ON u.id = c.user_id WHERE u.name = '李燕容') 
AND status != '課程已取消';
...
SELECT s.name AS 專長名稱, COUNT(c.id) AS coach_total
FROM "COACH" c
INNER JOIN "COACH_LINK_SKILL" cls ON c.id = cls.coach_id
INNER JOIN "SKILL" s ON cls.skill_id = s.id
GROUP BY s.name
...
SELECT cp.name AS 組合包方案名稱, COUNT(cp.id) AS 銷售數量
FROM "CREDIT_PURCHASE" cp
WHERE EXTRACT(MONTH FROM cp.purchase_at) = 11
GROUP BY cp.name;
...
WHERE EXTRACT(MONTH FROM cb.created_at) = 11 AND cb.status = '即將授課';